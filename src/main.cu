...
//HGEMM_FUNC(simdNaive);
//HGEMM_FUNC(simdOpt1);
//HGEMM_FUNC(simdOpt2);
//HGEMM_FUNC(simdOpt3);
//HGEMM_FUNC(simdWarpReorder);
//HGEMM_FUNC(simdWarpReorderOpt1);
//HGEMM_FUNC(simdWarpReorderOpt2);
//HGEMM_FUNC(simdWarpReorderOpt3);
//HGEMM_FUNC(simdWarpReorderOpt4);
//HGEMM_FUNC(simdWarpReorderOpt5);
//HGEMM_FUNC(simdWarpReorderOpt6);
//HGEMM_FUNC(simdWarpReorderOpt7);
...
DEFINE_bool(enable_simd, true, "test SIMD API");
...
int main(int argc, char *argv[]) {
    ...
    HLOG(
        "Profiling: enable wmma: %d, enable mma: %d, enable simd: %d, \
         warmup iterations: %u, profiling iterations: %u, sleep duration: "
        "%u ms, enable check: %d",
        FLAGS_enable_wmma, FLAGS_enable_mma, FLAGS_enable_simd,
        FLAGS_warmup_iterations, FLAGS_profiling_iterations, FLAGS_sleep_duration,
        FLAGS_enable_check);

    Tester tester(FLAGS_M, FLAGS_N, FLAGS_K, 
    ...
    //if (FLAGS_enable_simd) {
    //    tester.evaluate(simdNaive, "Simd-naive");
    //    tester.evaluate(simdOpt1, "Simd-opt1");
    //    tester.evaluate(simdOpt2, "Simd-opt2");
    //    tester.evaluate(simdOpt3, "Simd-opt3");
    //    tester.evaluate(simdWarpReorder, "Simd-warps_reorder");
    //    tester.evaluate(simdWarpReorderOpt1, "Simd-warps_reorder_opt1");
    //    tester.evaluate(simdWarpReorderOpt2, "Simd-warps_reorder_opt2");
    //    tester.evaluate(simdWarpReorderOpt3, "Simd-warps_reorder_opt3");
    //    tester.evaluate(simdWarpReorderOpt4, "Simd-warps_reorder_opt4");
    //    tester.evaluate(simdWarpReorderOpt5, "Simd-warps_reorder_opt5");
    //    tester.evaluate(simdWarpReorderOpt6, "Simd-warps_reorder_opt6");
    //    tester.evaluate(simdWarpReorderOpt7, "Simd-warps_reorder_opt7");
    //}
