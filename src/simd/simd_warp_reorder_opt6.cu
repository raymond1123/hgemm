...
__global__ void simdWarpReorderKernelOpt6(const half *__restrict__ A, 
            ...
            for(int i=0; i<MMA_M; ++i) { // i=0,1,2,...,7
                ...
                *((float4*)RA) = *(float4*)(&sharedA(iner_iter*BLOCK_ROWS+iner_row));
                ...
                for(int j=0; j<MMA_N; ++j) {
                    int iner_col = (i&1)?(warp_col_c+warp_tid_col+((MMA_N-1-j)<<3)):(warp_col_c+warp_tid_col+(j<<3));
...
size_t initSimdWarpReorderOpt6() {
    int dev_id = 0;
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(simdWarpReorderKernelOpt6, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void simdWarpReorderOpt6(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    size_t smem_max_size = initSimdWarpReorderOpt6();
    ...
    simdWarpReorderKernelOpt6<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
