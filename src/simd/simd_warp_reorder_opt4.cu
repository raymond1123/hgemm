...
//
// Description: simd warp reorder opt-4
...
__device__ void ldgstsAOpt4(half* sharedA, const half* tileA, 
...
__device__ void ldgstsBOpt4(half* sharedB, const half* tileB, 
...
__global__ void simdWarpReorderKernelOpt4(const half *__restrict__ A, 
    ...
    for(int iter_k=0; iter_k<K_tiles; ++iter_k) {
        /* LDG STS */
        ldgstsAOpt4(sharedA, tileA, 
                    tile_row_a, tile_col_a, 
                    smem_row_a, lda);
        ldgstsBOpt4(sharedB, tileB, 
...
size_t initSimdWarpReorderOpt4() {
    int dev_id = 0;
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(simdWarpReorderKernelOpt4, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void simdWarpReorderOpt4(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    size_t smem_max_size = initSimdWarpReorderOpt4();
    ...
    simdWarpReorderKernelOpt4<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
