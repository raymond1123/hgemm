...
//
// Description: simd warp reorder opt-1
...
__device__ void ldgstsAOpt1(half* sharedA, const half* tileA, 
...
__device__ void ldgstsBOpt1(half* sharedB, const half* tileB, 
...
__global__ void simdWarpReorderKernelOpt1(const half *__restrict__ A, 
    ...
    for(int iter_k=0; iter_k<K_tiles; ++iter_k) {
        /* LDG STS */
        ldgstsAOpt1(sharedA, tileA, row_a, col_a, lda);
        ldgstsBOpt1(sharedB, tileB, row_b, col_b, ldb);
        ...
        for(int iner_iter=0; iner_iter<CHUNK_K; ++iner_iter) {
            ...
            for(int i=0; i<MMA_N; ++i) {
                ...
                float4* tmp_R = (float4*)(&sharedB(warp_col_c+warp_tid_col+(i<<3), iner_iter*(AB_SMEM_STRIDE/2)));
...
size_t initSimdWarpReorderOpt1() {
    int dev_id = 0;
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(simdWarpReorderKernelOpt1, cudaFuncAttributeMaxDynamicSharedMemorySize, smem_max_size));

    return smem_max_size;
}

void simdWarpReorderOpt1(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    size_t smem_max_size = initSimdWarpReorderOpt1();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(div_ceil(N, BLOCK_COLS), div_ceil(M, BLOCK_ROWS));

    simdWarpReorderKernelOpt1<<<grid, block, smem_max_size>>>(A, B, C, M, N, K);
}
